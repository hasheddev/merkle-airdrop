...
# Remove modules
remove :; rm -rf .gitmodules && rm -rf .git/modules/* && rm -rf lib && touch .gitmodules && gid add . && git commit -m "modules"

install :
	forge install foundry-rs/forge-std --no-commit && forge install cyfrin/foundry-devops --no-commit && forge install openzeppelin/openzeppelin-contracts --no-commit && forge install dmfxyz/Murky --no-commit
